...
namespace TJS {
    ...
    static bool inline TJS_iswspace(tjs_char ch) {
        ...
        if (ch & 0xff00) return false;
        else return 0 != isspace(ch);
    }
...
#define TJS_STRINGAPPENDER_DATA_INC 8192
    ...
    void tTJSArrayNI::Assign(iTJSDispatch2* dsp) {
        ...
        if (TJS_SUCCEEDED(dsp->NativeInstanceSupport(TJS_NIS_GETINSTANCE,
            ...
            const auto p = std::make_unique<tTJSVariantClosure>(&callback, nullptr);
            dsp->EnumMembers(TJS_IGNOREPROP, p.get(), dsp);
        }
    }
	...
	stream.Write(TJS_W("(const) [\r\n"));
        ...
        const ttstr indentstr2 = indentstr + TJS_W(" ");

        tjs_uint c = 0;
        for (auto& Item : Items) {
            stream.Write(indentstr2);
            if (const tTJSVariantType type = Item.Type(); type == tvtObject) {
                // object
                tTJSVariantClosure clo = Item.AsObjectClosureNoAddRef();
                SaveStructuredDataForObject(clo.SelectObjectNoAddRef(),
                                            stack, stream, indentstr2);
            }
            else {
                stream.Write(TJSVariantToExpressionString(Item));
            }
#ifdef TJS_TEXT_OUT_CRLF
...
#define ARRAY_GET_VAL \
    ...
    tjs_int TJSCopyArrayElementTo(iTJSDispatch2* dsp,
        ...
        if (count < 0) count = static_cast<tjs_int>(ni->Items.size());

        if (start >= ni->Items.size()) return 0;

        const tjs_uint limit = start + count;

        for (tjs_uint i = start; i < limit; i++)
            *(dest++) = ni->Items[i];

        return static_cast<tjs_int>(limit - start);
    }
