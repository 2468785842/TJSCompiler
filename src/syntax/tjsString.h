...
#pragma pack(push, 4)
    ...
    public:
        tjs_int n;
        explicit tTJSStringBufferLength(const tjs_int n) { this->n = n; }
    };
#pragma pack(pop)
    /*]*/
    //---------------------------------------------------------------------------
...
#pragma pack(pop)
    ...
    public:
        //-------------------------------------------------------- constructor --
        TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString, ()): tTJSString_S() { Ptr = nullptr; }
        TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString, (const tTJSString &rhs)): tTJSString_S(rhs) {
            Ptr = rhs.Ptr;
            if (Ptr) Ptr->AddRef();
        }

        TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString, (tTJSVariantString *vstr)): tTJSString_S() {
            // NOLINT(*-explicit-constructor)
            Ptr = vstr;
            if (Ptr) Ptr->AddRef();
        }

        TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString, (const tjs_char *str)): tTJSString_S() {
            // NOLINT(*-explicit-constructor)
            Ptr = TJSAllocVariantString(str);
        }

        TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString, (const tjs_nchar *str)) : tTJSString_S() {
            // NOLINT(*-explicit-constructor)
            Ptr = TJSAllocVariantString(str);
        }

        TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, explicit tTJSString, (const tTJSStringBufferLength len)): tTJSString_S() {
            Ptr = TJSAllocVariantStringBuffer(len.n);
        }

        TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString, (const tjs_char rch)): tTJSString_S() {
            // NOLINT(*-explicit-constructor)
            tjs_char ch[2];
            ch[0] = rch;
            ch[1] = 0;
        ...
        TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString, (const tTJSVariant & val)); // NOLINT(*-explicit-constructor)

        TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString, (const tTJSString &str, const int n)): tTJSString_S()
        // construct with first n chars of str
        {
            Ptr = TJSAllocVariantString(str.c_str(), n);
        }

        TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString, (const tjs_char *str,const int n)): tTJSString_S()
        // same as above except for str's type
        {
            Ptr = TJSAllocVariantString(str, n);
        }

        TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, explicit tTJSString, (tjs_int n)); // from int
...
#endif
        tTJSString(const std::wstring& str): tTJSString_S() // NOLINT(*-explicit-constructor)
        {
	...
	const WideString AsWideString() const
	{
        ...
        [[nodiscard]] std::wstring AsStdString() const {
            if (!Ptr) return TJS_W("");
            return c_str();
        }
	...
	tTJSString & operator =(WideString &rhs)
	{
        ...
        TJS_METHOD_DEF(void, operator +=, (const tjs_char rch)) {
            Independ();
            tjs_char ch[2];
            ch[0] = rch;
            ch[1] = 0;
        ...
        TJS_CONST_METHOD_DEF(tTJSString, operator +, (const tjs_char rch)) {
            if (!Ptr) return rch;
            tjs_char ch[2];
            ch[0] = rch;
            ch[1] = 0;
        ...
        TJS_CONST_METHOD_DEF(tjs_char, operator [], (const tjs_uint i)) {
        ...
        TJS_METHOD_DEF(tjs_char *, AllocBuffer, (const tjs_uint len)) {
        ...
        TJS_METHOD_DEF(tjs_char *, AppendBuffer, (const tjs_uint len)) {
            /* you must call FixLen when you allocate larger buffer than actual string length */
        ...
        TJS_METHOD_DEF(tjs_uint32 *, GetHint, () const) {
    ...
    public:
        ...
        TJS_STATIC_METHOD_DEF(void *, operator new, (const size_t size)) { return new char[size]; }
        TJS_STATIC_METHOD_DEF(void, operator delete, (void * p)) { delete [] static_cast<char*>(p); }
