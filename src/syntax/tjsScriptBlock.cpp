...
namespace TJS {
    ...
    tTJSScriptBlock::tTJSScriptBlock(bool constparse) : ConstParse(true), Owner(NULL),
                                                        RefCount(1), Script(NULL), Name(NULL), InterCodeContext(NULL),
    ...
    tjs_uint tTJSScriptBlock::GetTotalVMCodeSize() const {
        ...
        for (auto interCodeContext : InterCodeContextList) {
            size += interCodeContext->GetCodeSize();
        }
        return size;
    }
...
extern tjs_uint time_ns_Commit;
    ...
    void tTJSScriptBlock::SetText(tTJSVariant* result, const tjs_char* text,
        ...
        Script = new tjs_char[wcslen(text) + 1];
		...
		tTJSTimeProfiler p(parsetime);
            ...
            if (!ConstParse) {
                ...
                if (!TopLevelContext->CopyConstData(result)) {
    ...
    tjs_int tTJSScriptBlock::GetCodeIndex(const tTJSInterCodeContext* ctx) const {
        tjs_int index = 0;
        for (std::list<tTJSInterCodeContext*>::const_iterator i = InterCodeContextList.begin(); i !=
             InterCodeContextList.end(); i++, index++) {
            if ((*i) == ctx) {
                return index;
            }
        }
...
#define OP1_DISASM TJS_OFFSET_VM_REG_ADDR(code[i+1]); size = 2;
            case VM_TT: OP1_DISASM;
                break;
            case VM_TF: OP1_DISASM;
                break;
            case VM_SETF: OP1_DISASM;
                break;
            case VM_SETNF: OP1_DISASM;
                break;
            case VM_LNOT: OP1_DISASM;
                break;
            case VM_BNOT: OP1_DISASM;
                break;
            case VM_ASC: OP1_DISASM;
                break;
            case VM_CHR: OP1_DISASM;
                break;
            case VM_NUM: OP1_DISASM;
                break;
            case VM_CHS: OP1_DISASM;
                break;
            case VM_CL: OP1_DISASM;
                break;
            case VM_INV: OP1_DISASM;
                break;
            case VM_CHKINV: OP1_DISASM;
                break;
            case VM_TYPEOF: OP1_DISASM;
                break;
            case VM_EVAL: OP1_DISASM;
                break;
            case VM_EEXP: OP1_DISASM;
                break;
            case VM_INT: OP1_DISASM;
                break;
            case VM_REAL: OP1_DISASM;
                break;
            case VM_STR: OP1_DISASM;
                break;
            case VM_OCTET: OP1_DISASM;
                break;
#undef OP1_DISASM
...
#define OP1A_DISASM TJS_OFFSET_VM_CODE_ADDR(code[i+1]); size = 2;
            case VM_JF: OP1A_DISASM;
                break;
            case VM_JNF: OP1A_DISASM;
                break;
            case VM_JMP: OP1A_DISASM;
                break;
#undef OP1A_DISASM
            ...
            case VM_REGMEMBER: size = 1;
                break;
            case VM_DEBUGGER: size = 1;
                break;
            default: size = 1;
                break;
            } /* switch */
