...
void tTJSDictionaryNI::Assign(iTJSDispatch2 * dsp, bool clear)
{
	...
	else
	{
		...
		auto p = std::make_unique<tTJSVariantClosure>(&countCallback, nullptr);
		...
		p = std::make_unique<tTJSVariantClosure>(&callback, nullptr);
		dsp->EnumMembers(TJS_IGNOREPROP, p.get(), dsp);
	...
	iTJSTextWriteStream & stream, const ttstr &indentstr)
{
	...
	Owner->EnumMembers(TJS_IGNOREPROP,p.get(), Owner);
