...
namespace TJS {
    ...
    //---------------------------------------------------------------------------
    tTJSNativeClassMethod::tTJSNativeClassMethod(const tTJSNativeClassMethodCallback& processfunc) {
        Process = processfunc;
        if (TJSObjectHashMapEnabled()) TJSAddObjectHashRecord(this);
    }

    tTJSNativeClassMethod::tTJSNativeClassMethod(const std::function<tjs_error()>& processfunc) {
        Process = [&processfunc](
            tTJSVariant*, tjs_int,
    ...
    tTJSNativeClass::tTJSNativeClass(const ttstr& name) {
        ...
        _ClassID = TJSRegisterNativeClass(name.c_str());
        TJSNativeClassSetClassID(this, _ClassID);
    }
