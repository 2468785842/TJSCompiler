...
#define TJS_GLOBAL_HASH_BITS 7
    ...
    tTJS::tTJS() {
        ...
        try {
            ...
            // Array
            iTJSDispatch2* dsp = new tTJSArrayClass(); //TJSCreateArrayClass();
    ...
    void tTJS::OutputToConsoleWithCentering(const tjs_char* msg, tjs_uint width) const {
        ...
        if (!msg) return;
        tjs_int len = (tjs_int)wcslen(msg);
    ...
    void tTJS::OutputToConsoleSeparator(const tjs_char* text, tjs_uint count) const {
        tjs_int len = (tjs_int)wcslen(text);
    ...
    void tTJS::Dump(tjs_uint width) const {
        ...
        if (ScriptBlocks.size()) {
            ...
            tjs_char buf[1024];
            TJS_snprintf(buf, sizeof(buf) / sizeof(tjs_char), TJS_W("Total %llu script block(s)"), ScriptBlocks.size());
            OutputToConsole(buf);
            OutputToConsole(TJS_W(""));
            ...
            TJS_snprintf(buf, sizeof(buf) / sizeof(tjs_char), TJS_W("Total count of contexts      : %d"),
                         totalcontexts);
            OutputToConsole(buf);
            TJS_snprintf(buf, sizeof(buf) / sizeof(tjs_char), TJS_W("Total VM code area size      : %d words"),
                         totalcodesize);
            OutputToConsole(buf);
            TJS_snprintf(buf, sizeof(buf) / sizeof(tjs_char), TJS_W("Total VM constant data count : %d"),
                         totaldatasize);
            OutputToConsole(buf);
            ...
            for (i = ScriptBlocks.begin(); i != ScriptBlocks.end(); i++) {
                ...
                if (name)
                    ...
                    title = TJS_W("(no-named script block)\0");
