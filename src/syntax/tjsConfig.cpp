...
#define TJS_MB_MAX_CHARLEN 2
    ...
    //---------------------------------------------------------------------------
    // 使われていないようなので未確認注意
    int TJS_wctomb(tjs_nchar *s, const tjs_char wc) {
        if (!s) return 0;
        const tjs_uint c = UnicodeToSJIS(wc);
        if (c == 0) return -1;
        int size = 0;
        if (c & 0xff00) {
    ...
    void TJSSetFPUE() {
        ...
        _MM_SET_EXCEPTION_MASK(
                _MM_MASK_INVALID | _MM_MASK_DIV_ZERO | _MM_MASK_DENORM | _MM_MASK_OVERFLOW | _MM_MASK_UNDERFLOW |
                _MM_MASK_INEXACT);
#else
        //	_fpreset();
