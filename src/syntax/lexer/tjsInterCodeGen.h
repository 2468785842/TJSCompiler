...
#undef TJS_NORMAL_AND_PROPERTY_ACCESSER
    //---------------------------------------------------------------------------
    enum tTJSSubType {
        stNone = VM_NOP, stEqual = VM_CP, stBitAND = VM_BAND, stBitOR = VM_BOR,
    ...
    public:
        ...
        tTJSVariant& GetValue() {
            if (!Val) return *static_cast<tTJSVariant*>(nullptr);
            return *Val;
        }
        ...
        [[nodiscard]] unsigned int GetSize() const {
            if (Nodes) return static_cast<unsigned int>(Nodes->size());
            return 0;
        }
    ...
    */
    ...
    private:
        enum tNestType {
            ntBlock, ntWhile, ntDoWhile, ntFor,
...
#define TJS_GNC_CFLAG_I (TJS_GNC_CFLAG+1) // inverted logic
    ...
    private:
        ...
        static void Add2ByteToVector(std::vector<tjs_uint8>* array, tjs_int16 value) {
            array->push_back((tjs_uint8)((value >> 0) & 0xff));
            array->push_back((tjs_uint8)((value >> 8) & 0xff));
        }
    ...
    public:
        ...
        tTJSInterCodeContext(tTJSScriptBlock* block, const tjs_char* name, tTJSContextType type,
                             ...
                             tjs_int arraybase, tjs_int colbase, bool srcsorted,
