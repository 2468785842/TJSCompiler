...
//---------------------------------------------------------------------------
namespace TJS {
    using enum parser::token_kind_type;
    //---------------------------------------------------------------------------
...
tjs_uint time_yylex = 0;
    ...
    //---------------------------------------------------------------------------
    int _yyerror(const tjs_char* msg, tTJSScriptBlock* pm, const tjs_int pos) {
        // handles errors that happen in the compilation

        tTJSScriptBlock* sb = pm;
    ...
    //---------------------------------------------------------------------------
    static tjs_int TJSGetContextHashSize(const tTJSContextType type) {
        switch (type) {
        case ctFunction:
        case ctExprFunction:
    ...
    //---------------------------------------------------------------------------
    tTJSInterCodeContext::tTJSInterCodeContext(tTJSInterCodeContext* parant,
	...
	if( Parent ) Parent->AddRef();
        ...
        if (name) {
            Name = new tjs_char[wcslen(name) + 1];
        ...
        try {
            ...
            {
            case ctTopLevel: VariableReserveCount = 2;
                break;
            case ctFunction: VariableReserveCount = 2;
                break;
            case ctExprFunction: VariableReserveCount = 2;
                break;
            case ctProperty: VariableReserveCount = 0;
                break;
            case ctPropertySetter: VariableReserveCount = 2;
                break;
            case ctPropertyGetter: VariableReserveCount = 2;
                break;
            case ctClass: VariableReserveCount = 2;
                break;
            case ctSuperClassGetter: VariableReserveCount = 2;
                break;
            }
    ...
    void tTJSInterCodeContext::Finalize(void) {
        if (PropSetter) PropSetter->Release(), PropSetter = nullptr;
        if (PropGetter) PropGetter->Release(), PropGetter = nullptr;
        if (SuperClassGetter) SuperClassGetter->Release(), SuperClassGetter = nullptr;
...
tjs_uint time_PutCode = 0;
    ...
    SortFunction(const void* a, const void* b) {
        const auto* aa = static_cast<const tSourcePos*>(a);
        const auto* bb = static_cast<const tSourcePos*>(b);
    ...
    void tTJSInterCodeContext::FixCode() {
        ...
        if (NonLocalFunctionDeclVector.size() >= 1) {
            ...
            for (func = NonLocalFunctionDeclVector.begin();
                 ...
                 ++func) {
                if (func->ChangeThis) codesize += 10;
                else codesize += 7;
            }
        ...
        for (auto& fix : FixList) {
            for (auto jmp = JumpList.begin(); jmp != JumpList.end();) {
                const tjs_int jmptarget = CodeArea[*jmp + 1] + *jmp;
            ...
            if (fix.NewSize > fix.Size) {
                ...
                CodeArea = static_cast<tjs_int32*>(TJS_realloc(
                    CodeArea,
                    sizeof(tjs_int32) * (CodeAreaSize + (fix.NewSize - fix.
                        Size))));
        ...
        for (const auto& jmp : JumpList) {
            tjs_int32 jumptarget = CodeArea[jmp + 1] + jmp;
            const tjs_int32 jumpcode = CodeArea[jmp];
            tjs_int addr = jmp;
    ...
    void tTJSInterCodeContext::RegisterFunction() {
        ...
        if (Parent->ContextType == ctFunction ||
            ...
            if (IsBytecodeCompile) {
                // for bytecode export
                if (Properties == nullptr) Properties = new std::vector<tProperty*>();
    ...
    tjs_int tTJSInterCodeContext::GenNodeCode(tjs_int& frame, tTJSExprNode* node,
        ...
        case T_RBITSHIFTEQUAL: // '>>>=' operator
        {
            ...
            {
            case T_AMPERSANDEQUAL: param2.SubType = stBitAND;
                break;
            case T_VERTLINEEQUAL: param2.SubType = stBitOR;
                break;
            case T_CHEVRONEQUAL: param2.SubType = stBitXOR;
                break;
            case T_MINUSEQUAL: param2.SubType = stSub;
                break;
            case T_PLUSEQUAL: param2.SubType = stAdd;
                break;
            case T_PERCENTEQUAL: param2.SubType = stMod;
                break;
            case T_SLASHEQUAL: param2.SubType = stDiv;
                break;
            case T_BACKSLASHEQUAL: param2.SubType = stIDiv;
                break;
            case T_ASTERISKEQUAL: param2.SubType = stMul;
                break;
            case T_LOGICALOREQUAL: param2.SubType = stLogOR;
                break;
            case T_LOGICALANDEQUAL: param2.SubType = stLogAND;
                break;
            case T_RARITHSHIFTEQUAL: param2.SubType = stSAR;
                break;
            case T_LARITHSHIFTEQUAL: param2.SubType = stSAL;
                break;
            case T_RBITSHIFTEQUAL: param2.SubType = stSR;
                break;
            }
            param2.SubAddress = resaddr;
        ...
        case T_ASTERISK: // binary '*' operator
        {
            ...
            {
            case T_VERTLINE: code = VM_BOR;
                break;
            case T_CHEVRON: code = VM_BXOR;
                break;
            case T_AMPERSAND: code = VM_BAND;
                break;
            case T_RARITHSHIFT: code = VM_SAR;
                break;
            case T_LARITHSHIFT: code = VM_SAL;
                break;
            case T_RBITSHIFT: code = VM_SR;
                break;
            case T_PLUS: code = VM_ADD;
                break;
            case T_MINUS: code = VM_SUB;
                break;
            case T_PERCENT: code = VM_MOD;
                break;
            case T_SLASH: code = VM_DIV;
                break;
            case T_BACKSLASH: code = VM_IDIV;
                break;
            case T_ASTERISK: code = VM_MUL;
                break;
            }
        ...
        case T_GTOREQUAL: // '>=' operator
        {
            ...
            switch (node->GetOpecode()) // ...
            {
            case T_NOTEQUAL: code1 = VM_CEQ;
                code2 = VM_SETNF;
                break;
            case T_EQUALEQUAL: code1 = VM_CEQ;
                code2 = VM_SETF;
                break;
            case T_DISCNOTEQUAL: code1 = VM_CDEQ;
                code2 = VM_SETNF;
                break;
            case T_DISCEQUAL: code1 = VM_CDEQ;
                code2 = VM_SETF;
                break;
            case T_LT: code1 = VM_CLT;
                code2 = VM_SETF;
                break;
            case T_GT: code1 = VM_CGT;
                code2 = VM_SETF;
                break;
            case T_LTOREQUAL: code1 = VM_CGT;
                code2 = VM_SETNF;
                break;
            case T_GTOREQUAL: code1 = VM_CLT;
                code2 = VM_SETNF;
                break;
            }
        ...
        case T_OCTET: // 'octet' operator
        {
            ...
            case T_TILDE: code = VM_BNOT;
                break;
            case T_SHARP: code = VM_ASC;
                break;
            case T_DOLLAR: code = VM_CHR;
                break;
            case T_UPLUS: code = VM_NUM;
                break;
            case T_UMINUS: code = VM_CHS;
                break;
            case T_INVALIDATE: code = VM_INV;
                break;
            case T_ISVALID: code = VM_CHKINV;
                break;
            case T_TYPEOF: code = VM_TYPEOF;
                break;
            case T_EVAL: code = (restype & TJS_RT_NEEDED) ? VM_EVAL : VM_EEXP;
            ...
            // warn if T_EVAL is used in non-global position
                ...
                break;
            case T_INT: code = VM_INT;
                break;
            case T_REAL: code = VM_REAL;
                break;
            case T_STRING: code = VM_STR;
                break;
            case T_OCTET: code = VM_OCTET;
                break;
            }
            PutCode(code, node_pos);
            PutCode(TJS_TO_VM_REG_ADDR(resaddr), node_pos);
        ...
        case T_POSTDECREMENT: // post-positioned '--' operator
        {
            ...
            tSubParam param2;
            switch (node->GetOpecode()) {
            case T_TYPEOF: param2.SubType = stTypeOf;
                break;
            case T_DELETE: param2.SubType = stDelete;
                break;
            case T_INCREMENT: param2.SubType = stPreInc;
                break;
            case T_DECREMENT: param2.SubType = stPreDec;
                break;
            case parser::token_kind_type::T_POSTINCREMENT: param2.SubType = stPostInc;
                break;
            case T_POSTDECREMENT: param2.SubType = stPostDec;
                break;
            }
...
tjs_uint time_make_np = 0;
    ...
    bool tTJSInterCodeContext::CopyConstData(tTJSVariant* result) {
        if (DataArea != nullptr && DataAreaSize == 1 &&
