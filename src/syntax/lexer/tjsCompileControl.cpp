...
namespace TJSPP {
    int yylex(parser::value_type* yylex, tTJSPPExprParser* ptr) {
        ...
        if (n == parser::token_kind_type::PT_NUM) yylex->val = val;
        if (n == parser::token_kind_type::PT_SYMBOL) yylex->nv = val;
        return n;
    }
    ...
    tjs_int tTJSPPExprParser::GetNext(tjs_int32& value) {
        ...
        case TJS_W('9'): {
            ...
            tTJSVariant val;
            try {
                if (!TJSParseNumber(val, &Current)) return parser::token::PT_ERROR;
            }
            catch (...) {
                return parser::token::PT_ERROR;
            }
            value = static_cast<tjs_int32>(val);
        ...
        const ttstr str{st, static_cast<int>(Current - st)};

        IDs.push_back(str);
        value = static_cast<tjs_int32>(IDs.size() - 1);

        return parser::token::PT_SYMBOL;
    }

    //---------------------------------------------------------------------------
    const tjs_char* tTJSPPExprParser::GetString(const tjs_int idx) const {
