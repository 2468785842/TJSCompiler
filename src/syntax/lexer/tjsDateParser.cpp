...
namespace TJSDate {
    ...
    //---------------------------------------------------------------------------
    constexpr static bool TJS_iswspace(tjs_char ch) {
        if (ch & 0xff00) return false;
        return isspace(ch);
    }

    //---------------------------------------------------------------------------
    constexpr static bool TJS_iswdigit(tjs_char ch) {
        if (ch & 0xff00) return false;
        return isdigit(ch);
    }

    //---------------------------------------------------------------------------
    constexpr static bool TJS_iswalpha(tjs_char ch) {
    ...
    tTJSDateParser::tTJSDateParser(const tjs_char* in) {
        ...
        if (parser{this}.parse()) TJS_eTJSError(TJSCannotParseDate);
    ...
    = default;
    //---------------------------------------------------------------------------
    tjs_int64 tTJSDateParser::GetTime() const {
        return Time;
    }
