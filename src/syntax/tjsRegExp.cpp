...
namespace TJS {
    ...
    //---------------------------------------------------------------------------
    // some TJS flags
    const tjs_uint32 globalsearch = static_cast<tjs_uint32>(1) << 31;
    const tjs_uint32 tjsflagsmask = 0xff000000;
    ...
    void replace_regex(tTJSVariant** param, tjs_int numparams, tTJSNI_RegExp* _this, iTJSDispatch2* objthis,
        ...
        if (r >= 0) {
            ...
            } while (isreplaceall && s < send && onig_search(_this->RegEx, (UChar*)s, (UChar*)send, (UChar*)s,
                                                             (UChar*)send, region, ONIG_OPTION_NONE) >= 0);
            if (s < send) {
    ...
    void tTJSNC_RegExp::Compile(tjs_int numparams, tTJSVariant** param, tTJSNI_RegExp* _this) {
        ...
        OnigErrorInfo einfo;
        if (int r = onig_new(&(_this->RegEx), (UChar*)expr.c_str(), (UChar*)(expr.c_str() + expr.length()),
                             flags & ((ONIG_OPTION_MAXBIT << 1) - 1), ONIG_ENCODING_UTF16_LE, ONIG_SYNTAX_PERL,
                             &einfo)) {
            char s[ONIG_MAX_ERROR_MESSAGE_LEN];
            onig_error_code_to_str((UChar*)s, r, &einfo);
            TJS_eTJSError(s);
    ...
    bool tTJSNC_RegExp::Exec(OnigRegion* region, const ttstr& target, tTJSNI_RegExp* _this) {
        ...
        if (!matched || region->num_regs <= 0) {
            ...
            _this->LastMatch = ttstr(target.c_str() + _this->Start + (region->beg[0] / sizeof(tjs_char)),
                                     (region->end[0] - region->beg[0]) / sizeof(tjs_char));
            tjs_uint last = num_regs - 1;
            _this->LastParen = ttstr(target.c_str() + _this->Start + (region->beg[last] / sizeof(tjs_char)),
            ...
            if (region->num_regs <= 0) {
                ...
                try {
                    for (tjs_uint i = 0; i < size; i++) {
                        tTJSVariant val;
                        val = ttstr(target + _this->Start + (region->beg[i] / sizeof(tjs_char)),
