...
public:
	static tjs_uint32 Make(const T &val)
	{
		...
		const char *plim = reinterpret_cast<const char*>(&val) + sizeof(T);
		tjs_uint32 ret = 0;
		while(p<plim)
		{
			ret += *p;
		...
		ret ^= (ret >> 11);
		ret += (ret << 15);
		if(!ret) ret = static_cast<tjs_uint32>(-1);
		return ret;
	}
};
//---------------------------------------------------------------------------
...
//		{ elm = ref.elm; }
		...
		KeyT & GetKey()
		{ return *reinterpret_cast<KeyT*>(elm->Key); }

		ValueT & GetValue()
		{ return *reinterpret_cast<ValueT*>(elm->Value); }
	...
	bool FindWithHash(const KeyT &key, tjs_uint32 hash, const KeyT *& keyout, ValueT *& value) const
	{
		...
		hash = 0;
#endif
		if(const element * elm = InternalFindWithHash(key, hash))
		{
			value = static_cast<ValueT*>(elm->Value);
			keyout = static_cast<const KeyT*>(elm->Key);
			return true;
		}
		return false;
	}
	...
	ValueT * FindAndTouchWithHash(const KeyT &key, tjs_uint32 hash)
	{
		...
		CheckUpdateElementOrder(const_cast<element*>(elm)); // force casting
		return reinterpret_cast<ValueT*>(const_cast<char*>(elm->Value));
	}
	...
	bool FindAndTouchWithHash(const KeyT &key, tjs_uint32 hash, const KeyT *& keyout, ValueT *& value)
	{
		...
		if(const element * elm = InternalFindWithHash(key, hash))
		{
			CheckUpdateElementOrder(const_cast<element*>(elm));
			value = reinterpret_cast<ValueT*>(const_cast<char*>(elm->Value));
			keyout = reinterpret_cast<const KeyT*>(elm->Key);
			return true;
		}
		return false;
	}
	...
	bool DeleteWithHash(const KeyT &key, tjs_uint32 hash)
	{
		...
		{
			if(key == *reinterpret_cast<KeyT*>(lv1->Key))
			{
				// delete lv1
		...
		while(elm)
		{
			if(hash == elm->Hash)
			{
				if(key == *reinterpret_cast<KeyT*>(elm->Key))
				{
	...
	[[nodiscard]] tjs_uint GetCount() const { return Count; }
...
public:
	tTJSHashCache(const tjs_uint maxcount) { MaxCount = maxcount; }
	...
	[[nodiscard]] tjs_uint GetMaxCount() const { return MaxCount; }
};
//---------------------------------------------------------------------------
} // namespace TJS
//---------------------------------------------------------------------------
