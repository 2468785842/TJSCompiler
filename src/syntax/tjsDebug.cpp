...
namespace TJS {
    ...
    public:
        ...
        void WarnIfObjectIsDeleting(iTJSConsoleOutput* output, void* object) {
            ...
            if (rec->Flags & TJS_OHMF_DELETING) {
                ...
                tjs_char tmp[64];
                TJS_snprintf(tmp, sizeof(tmp) / sizeof(tjs_char), TJS_W("0x%p"),
                             object);
        ...
        void ReportAllUnfreedObjects(iTJSConsoleOutput* output) {
            {
                ttstr msg{TJSNObjectsWasNotFreed};
                msg.Replace(TJS_W("%1"), ttstr((tjs_int)Hash.GetCount()));
                output->Print(msg.c_str());
            }

            // list all unfreed objects
            for (auto i = Hash.GetFirst(); !i.IsNull(); ++i) {
                tjs_char addr[65];
                TJS_snprintf(addr, sizeof(addr) / sizeof(tjs_char), TJS_W("0x%p"),
                             i.GetKey());
                ttstr info {TJSObjectWasNotFreed};
            ...
            output->Print(TJS_W("---"));
            output->Print(TJSGroupByObjectTypeAndHistory);
            ...
            if (!items.empty()) {
                ...
                for (auto i = items.begin();; ++i) {
                    if (i != items.begin() &&
                        (i == items.end() || history != i->History || type != i->
                            Type)) {
                        tjs_char tmp[64];
                        TJS_snprintf(tmp, sizeof(tmp) / sizeof(tjs_char),
                                     TJS_W("%6d"), (int)count);
                        ttstr info{
            ...
            output->Print(TJS_W("---"));
            output->Print(TJSGroupByObjectType);
            ranges::stable_sort(items,
            ...
            if (!items.empty()) {
                for (auto i = items.begin();
                    ...
                    if (i != items.begin() &&
                        ...
                        TJS_snprintf(tmp, sizeof(tmp) / sizeof(tjs_char),
                                     TJS_W("%6d"), count);
                        ttstr info{TJSObjectCountingMessageTJSGroupByObjectType};
                        info.Replace(TJS_W("%1"), tmp);
                        info.Replace(TJS_W("%2"), type);
                        output->Print(info.c_str());
        ...
        void ReplayLog() {
            // replay recorded log
            while (true) {
                tTJSObjectHashMapLogItemId id = TJSRestoreLog<
                    tTJSObjectHashMapLogItemId>();
    ...
    public:
        ...
        void Push(tTJSInterCodeContext* context, bool in_try) {
            Stack.emplace_back(context, in_try);
        }
...
public:
	...
	void PrintLog( const tjs_char* mes, bool important ) {
		if( DebuggerHwnd != INVALID_HANDLE_VALUE ) {
			...
			DebuggerMessage	message( DBGEV_GEE_LOG, (void*)mes, (wcslen(mes) + 1) * sizeof(tjs_char) );
