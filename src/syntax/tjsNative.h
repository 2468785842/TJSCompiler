...
namespace TJS {
    ...
    typedef std::function<
        tjs_error(tTJSVariant*, tjs_int, tTJSVariant**, iTJSDispatch2*)
    > tTJSNativeClassMethodCallback;
    /*]*/
#ifdef __TP_STUB_H__
/*[*/
...
/*]*/
    ...
    tTJSNativeClassMethod* TJSCreateNativeClassMethod(
        const tTJSNativeClassMethodCallback& callback
    );

    tTJSNativeClassMethod* TJSCreateNativeClassMethod(
        const std::function<tjs_error()>& callback
    );
    ...
    public:
        tTJSNativeClassMethod(const tTJSNativeClassMethodCallback& processfunc);
        tTJSNativeClassMethod(const std::function<tjs_error()>& processfunc);
        ~tTJSNativeClassMethod();
    ...
    public:
        tTJSNativeClassConstructor(const tTJSNativeClassMethodCallback& processfunc)
            : tTJSNativeClassMethod(processfunc) {
        }

        tTJSNativeClassConstructor(const std::function<tjs_error()>& processfunc)
            : tTJSNativeClassMethod(processfunc) {
        }
...
/*]*/
    ...
    inline void TJSNativeClassRegisterNCM(
        ...
        const tTJSNativeInstanceType type,
        const tjs_uint32 flags = 0) {
        cls->RegisterNCM(name, dsp, classname.c_str(), type, flags);
    }
...
#define TJS_PARAM_EXIST(num) (numparams>(num) ? param[num]->Type()!=tvtVoid : false)
    ...
    protected:
        virtual tjs_error Process(tTJSVariant* result, tjs_int numparams,
