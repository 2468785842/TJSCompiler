...
#define TJSVS_ALLOC_DOUBLE_LIMIT 4000000
    ...
    tjs_char* TJSVS_malloc(tjs_uint len) {
        len += TJSVS_ALLOC_INC_SIZE_S;
        auto* ret = new char[len * sizeof(tjs_char) + sizeof(size_t)];
        // 将大小赋值在内存开始段
        *reinterpret_cast<size_t*>(ret) = len;
    ...
    tjs_char* TJSVS_realloc(tjs_char* buf, tjs_uint len) {
        ...
        const auto* ptr = reinterpret_cast<size_t*>(reinterpret_cast<char*>(buf) - sizeof(size_t));
    ...
    void TJSVS_free(const tjs_char* buf) {
        if (!buf) return;
        const auto* ptr = reinterpret_cast<const char*>(buf) - sizeof(size_t);
        delete[] ptr;
    }
...
#define HEAP_CAPACITY_INC 4096
    static tTJSCriticalSection* TJSStringHeapCS = nullptr;
...
#define TJS_CHECK_FREE_BLOCK_MAX 50
    ...
    tTJSVariantString* tTJSVariantString::FixLength() {
        ...
        Length = (tjs_int)wcslen(this->operator const tjs_char*());
        if (!Length) {
            TJSDeallocStringHeap(this);
            return NULL;
        }
...
#define check_alloc \
        ...
        type_char:
            ...
            case TJS_W('X'): {
                ...
                tjs_uint size = (tjs_uint)wcslen(buf);
                tjs_uint inc_size;
                if (s + size > allocsize) {
